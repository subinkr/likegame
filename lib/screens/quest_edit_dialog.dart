...
class QuestEditDialog extends ConsumerStatefulWidget {
  final Quest quest;
  final QuestService questService;
  final Function() onQuestUpdated;
  ...
  @override
  ConsumerState<QuestEditDialog> createState() => _QuestEditDialogState();
}

class _QuestEditDialogState extends ConsumerState<QuestEditDialog> {
  ...
  void initState() {
    super.initState();
    titleController = TextEditingController(text: widget.quest.title);
    descriptionController = TextEditingController(text: widget.quest.description ?? '');
    subTaskController = TextEditingController();
    
    selectedDueDate = widget.quest.dueDate;
    selectedPriority = widget.quest.priority;
    selectedDifficulty = widget.quest.difficulty;
    subTasks = List.from(widget.quest.subTasks.map((task) => task.title));
  }

  // UserProvider 접근 부분 추가
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: _isLoading ? null : _updateQuest,
                    child: _isLoading
                        ? const SizedBox(
                            height: 20,
                            width: 20,
